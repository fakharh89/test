//
//  ASEnvironmentalAlertIntervalCharacteristic.m
//  Pods
//
...
@interface ASEnvironmentalAlertIntervalCharacteristic ()
...
@property (strong, readwrite, nonatomic) NSNumber *writtenAlertInterval;

@end

@implementation ASEnvironmentalAlertIntervalCharacteristic
...
- (BOOL)updateDeviceWithData:(NSNumber *)data error:(NSError *__autoreleasing *)error {
    self.device.alertInterval = data;
    ASLog(@"Read alert interval: %llu", [data unsignedLongLongValue]);
    return YES;
}
...
- (void)write:(NSNumber *)alertInterval withCompletion:(void (^)(NSError *error))completion {
    ...
    uint32_t num = alertInterval.unsignedIntValue;
    NSData *rawData = [NSData dataWithBytes:&num length:sizeof(num)];
    
    self.writtenAlertInterval = alertInterval;
...
- (void)didCompleteWriteWithError:(NSError *)error {
    if (self.writeCompletion) {
        ...
        if (!error) {
            [self updateDeviceWithData:self.writtenAlertInterval error:&error];
        }
        
        self.writtenAlertInterval = nil;
