//
//  ASActivityDataCharacteristic.m
//  Pods
//
...
@interface ASActivityDataCharacteristic ()
...
@implementation ASActivityDataCharacteristic
...
- (BOOL)updateDeviceWithData:(ASActivityState *)data error:(NSError *__autoreleasing *)error {
    ...
    NSTimeInterval timeInterval = [data.date timeIntervalSinceDate:self.device.container.activityStates.lastObject.date];
    if (timeInterval < 0) {
    ...
    [self.device.container addNewActivityMeasurement:data];
    
    ASLog(@"Activity data: %@", data);
...
- (ASBLEResult<ASActivityState *> *)process {
    ...
    return [data as_activityStateWithFirmwareVersion:self.device.softwareRevision];
}
...
- (void)processUpdateDeviceAndSendNotificationWithError:(NSError *)error {
    ...
    ASBLEResult<ASActivityState *> *result = [self process];
