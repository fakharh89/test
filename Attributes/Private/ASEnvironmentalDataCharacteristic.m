//
//  ASEnvironmentalDataCharacteristic.m
//  Pods
//
...
@interface ASEnvironmentalDataCharacteristic ()
...
@implementation ASEnvironmentalDataCharacteristic
...
- (BOOL)updateDeviceWithData:(ASEnvironmentalMeasurement *)data error:(NSError *__autoreleasing *)error {
    ...
    NSTimeInterval timeInterval = [data.date timeIntervalSinceDate:self.device.container.environmentalMeasurements.lastObject.date];
    if (timeInterval < 0) {
    ...
    ASLog(@"Env Data for %@: Date - %@, Humidity - %@, Temperature - %@, Diff - %0.3f", self.device.container.name ?: self.device.container.identifier, data.date, data.humidity, data.temperature, timeInterval);
    
    [self.device.container addNewEnvironmentalMeasurement:data];
...
- (ASBLEResult<ASEnvironmentalMeasurement *> *)process {
    ...
    return [data as_environmentalMeasurementWithFirmwareVersion:self.device.softwareRevision];
}
...
- (void)processUpdateDeviceAndSendNotificationWithError:(NSError *)error {
    ...
    ASBLEResult<ASEnvironmentalMeasurement *> *result = [self process];
