//
//  ASActivityBufferCharacteristic.m
//  Pods
//
...
@interface ASActivityBufferCharacteristic ()
...
@implementation ASActivityBufferCharacteristic
...
- (ASBLEResult<NSArray<ASBLEResult<ASActivityState *> *> *> *)process {
    ...
    return [data as_activityBufferWithFirmwareVersion:self.device.softwareRevision];
}
...
- (BOOL)updateDeviceWithMeasurement:(ASActivityState *)measurement error:(NSError * __autoreleasing *)error {
    NSParameterAssert(measurement);
    
    // Make sure we didn't go back in time
    NSTimeInterval timeInterval = [measurement.date timeIntervalSinceDate:self.device.container.activityStates.lastObject.date];
    if (timeInterval < 0) {
        ASLog(@"Bad activity packet: Packet went back in time");
        if (error) {
        ...
        ASLog(@"Bad activity packet: Packet is same as last data added");
        if (error) {
    ...
    [self.device.container addNewActivityMeasurement:measurement];
    
    ASLog(@"Activity data: %@", measurement);
