//
//  ASImpactBufferCharacteristic.m
//  Pods
//
...
@interface ASImpactBufferCharacteristic ()
...
@implementation ASImpactBufferCharacteristic
...
- (ASBLEResult<NSArray<ASBLEResult<ASImpact *> *> *> *)process {
    ...
    return [data as_impactBufferWithFirmwareVersion:self.device.softwareRevision];
}
...
- (BOOL)updateDeviceWithMeasurement:(ASImpact *)measurement error:(NSError * __autoreleasing *)error {
    ...
    NSTimeInterval timeInterval = [measurement.date timeIntervalSinceDate:self.device.container.impacts.lastObject.date];
    if (timeInterval < 0) {
    ...
    [self.device.container addNewImpactMeasurement:measurement];
    
    ASLog(@"Impact data %@", measurement);
